...
public class Texture {
	...
	public void customMipMap(String resourceName, int mipMap) throws IOException {
		BufferedImage image = ImageIO.read(createInputStreamFromResourceName(resourceName));
		int width = image.getWidth();
		int height = image.getHeight();
		boolean hasAlpha = image.getColorModel().hasAlpha();
		...
		glTexImage2D(GL_TEXTURE_2D, mipMap, hasAlpha ? GL_RGBA : GL_RGB, width, height, 0, hasAlpha ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE, buffer);
	}
